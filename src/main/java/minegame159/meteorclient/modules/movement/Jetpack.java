...
public class Jetpack extends Module {
    ...
    public Jetpack() {
        super(Category.Movement, "Jetpack", "Fly like a jet I guess");
    }
    	
    private final SettingGroup sgGeneral = settings.getDefaultGroup();
    private final SettingGroup sgAntiKick = settings.createGroup("Anti Kick"); //Pog
    private final SettingGroup sgDontTouch = settings.createGroup("Not Here");

    private final Setting<Mode> mode = sgGeneral.add(new EnumSetting.Builder<Mode>()
            .name("mode")
            .description("The mode for Jetpack.")
    ...
    private final Setting<Double> speed = sgGeneral.add(new DoubleSetting.Builder()
            .name("speed")
            .description("Your speed when jetpacking.")
            .defaultValue(0.1)
            .min(0.0)
            .build()
    );
    ...
    private final Setting<Boolean> verticalSpeedMatch = sgDontTouch.add(new BoolSetting.Builder()
            .name("Dont turn off")
            .description("Does math and breaks jetpack if turned off")
            .defaultValue(true)
            .build()
    );
